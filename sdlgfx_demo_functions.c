...
void demo_text_mathematical_waltz(void);
...
void demo_text_mathematical_waltz() {
    sdlgfx_clear();
    Uint32 start_time = SDL_GetTicks();
    const char* text = "Math123 Dance456";
    ...
    while (running && (SDL_GetTicks() - start_time < DEMO_DURATION)) {
        ...
        float time = SDL_GetTicks() * 0.001f; // Время для анимации
        int center_x = SCREEN_WIDTH / 2;
        int center_y = SCREEN_HEIGHT / 2;

        // Каждый символ танцует по своей орбите
        for (int i = 0; i < text_len; i++) {
            float angle = time + i * 0.5f; // Уникальный угол для каждого символа
            float radius = 180 + 50 * sin(time * 0.3f + i); // Радиус орбиты пульсирует
            int x = center_x + (int)(radius * cos(angle));
            int y = center_y + (int)(radius * sin(angle * 1.2f)); // Немного искажаем орбиту

            // Цвета зависят от позиции и времени
            int r = (int)(127 * sin(angle + time) + 128);
            int g = (int)(127 * cos(angle + time * 0.7f) + 128);
            int b = (int)(127 * sin(angle + time * 1.3f) + 128);

            sdlgfx_color(r, g, b);

            // Выводим один символ с небольшим "вальсирующим" смещением
            char single_char[2] = {text[i], '\0'};
            sdlgfx_string(x - FONT_WIDTH / 2, y - FONT_HEIGHT / 2, single_char);
        }

        draw_info_panel("sdlgfx_string", "Mathematical Waltz of Letters and Numbers",
                       DEMO_DURATION - (SDL_GetTicks() - start_time));
        sdlgfx_flush();
        SDL_Delay(16);
        handle_input(&running);
...
//	if (!sdlfont_load_psf("ter-u16b.psf"));
